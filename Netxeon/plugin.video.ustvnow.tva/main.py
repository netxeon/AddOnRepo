...
if mode == 'main':
    ...
    if channels:
        for c in channels:
            ...
            Addon.add_video_item(rURL,
                                 ...
                                 img=poster_url, fanart=logo, HD=quality_name, playable=c['playable'], cm=cm_menu, cm_replace=cm_view)
    ...
    if recordings:
        for r in sorted(recordings, reverse=True):
            ...
            Addon.add_video_item(rURL, {'title': title, 
                                 ...
                                 img=poster_url, fanart=logo, HD=quality_name, cm=cm_menu, cm_replace=True, playable=r['playable'])
        xbmcplugin.setContent(Addon.plugin_handle, 'movie')
    ...
    if recordings:
        for r in sorted(recordings, reverse=True):
            ...
            Addon.add_list_item(r['title'], {'title': title, 'plot': plot, 
                                                   'dateadded': r['rec_date']},
                                 img=poster_url, fanart=logo, HD=quality_name, cm=cm_menu, cm_replace=True, playable=False)
        xbmcplugin.setContent(Addon.plugin_handle, 'movie')
    ...
    if now:
        for r in now:
            channel = r['channel']
            rURL = "plugin://plugin.video.ustvnow.tva/?name=" + r['channel'] + "&mode=play&rand=" + Addon.random_generator()
            if r['channel'] == 'AE':
                channel = 'A&E'
            if r['channel'] == 'MY9':
                channel = 'My9'
    ...
    if channels:
        for c in channels:
            ...
            Addon.add_video_item(rURL,
                                 ...
                                 'mediatype': mediatype,
                                 'tvshowtitle': tvshowtitle},
                                 img=poster_url, fanart=logo, HD=quality_name, playable=c['playable'], cm=cm_menu, cm_replace=True)
            xbmcplugin.setContent(Addon.plugin_handle, 'movie')
    ...
    if channels:
        for c in channels:
            ...
            Addon.add_list_item(c['title'],
                                 ...
                                 'mediatype': mediatype,
                                 'tvshowtitle': tvshowtitle},
                                 img=poster_url, fanart=logo, HD=quality_name, playable=False, cm=cm_menu, cm_replace=True)
            xbmcplugin.setContent(Addon.plugin_handle, 'movie')
    ...
    try:
        ...
        if listings:
            for l in range(len(listings)):
                ...
                if listings[l][11] == '0':
                    ...
                    playable = True
                rURL = "plugin://plugin.video.ustvnow.tva/?name="+listings[l][0]+"&mode=play&rand=" + Addon.random_generator()
                if listings[l][4] == '\n':
                ...
                Addon.add_video_item(rURL,
                                     ...
                                     img=listings[l][6].replace(' ','%20') , playable=playable, HD=quality_name, cm=cm_menu, cm_replace=True)
        ...
        if Addon.makeXMLTV(ustv.get_guidedata(quality_type),urllib.unquote(fpath)) == True:
            ...
            if listings:
                for l in range(len(listings)):
                    url = "plugin://plugin.video.ustvnow.tva/?name="+listings[l]+"&mode=tvguide_list"
