...
class Ustvnow:
    ...
    def build_main(self):
        ...
        if Addon.get_setting('show_tv_guide_list') == 'true':
            Addon.add_directory({'mode': 'tvguide_list'}, Addon.get_string(40019))
    ...
    def get_guidedata(self, quality):
	    ...
	    passkey = self._get_passkey()
	    self._stream_check()
	    content = self._get_json('gtv/1/live/channelguide', {'c': '','l': '1440', 'token': self.token})
	    results = content['results'];
	    now = time();
