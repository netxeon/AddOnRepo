import urllib,re,xbmcplugin,xbmcgui,xbmc,xbmcaddon,os,xbmcvfs,shutil,hashlib
import requests
...
key3 = "hkkhn1fxwi7ps8psw5vjcomu1gk58bo8"
s = requests.session()
net = Net()
...
def EPIS(url):
        ...
        video_id = re.compile('movie_id" value="(.*?)"').findall(link)[0]
        ...
        for a in all_videos:
                ...
                headers = referer + '\+' + coookie_1 + '=' + coookie_2
                url =  baseurl + '/ajax/v2_load_episode/'+episode_id+'/'+coookie_3
...
def LINKS(url):
        ...
        #try:
                ...
                #request_url =  baseurl + '/ajax/get_episodes/'+video_id+'/'+token
        #except:
        video_id = re.compile('movie_id" value="(.*?)"').findall(link)[0]
        ...
        try:
                ...
                for a in all_videos:
                        ...
                        request_url2 =  baseurl + '/ajax/v2_load_episode/'+episode_id+'/'+coookie_3
                        headers = {'Accept-Encoding':'gzip, deflate, sdch', 'Referer': referer, 'user-agent':User_Agent,'x-requested-with':'XMLHttpRequest'}
...
def LINKS2(url,description):
        ...
        headers = {'Referer': referer, 'user-agent':User_Agent,'x-requested-with':'XMLHttpRequest'}
